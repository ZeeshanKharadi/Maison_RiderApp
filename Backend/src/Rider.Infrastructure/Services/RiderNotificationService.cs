    ...
    public class RiderNotificationService : IRiderNotificationService
    {
            ...
            SendNotificationRequest request)
        {
            if (request == null || request.userId == Guid.Empty)
                return FailResult("userId is required");

            var user = await _unitOfWork.UserRepository.GetByUserIdAsync(request.userId);
            if (user == null)
                return FailResult("User not found");

            var title = string.IsNullOrWhiteSpace(request.title) ? "Test notification" : request.title.Trim();
            var body = string.IsNullOrWhiteSpace(request.body) ? "Hello from Maison Rider API" : request.body.Trim();
            var category = string.IsNullOrWhiteSpace(request.category) ? "system" : request.category.Trim();
        ...
        public async Task<ApiResponse<SendNotificationResultDto>> BroadcastTestAsync(
            BroadcastNotificationRequest request)
        {
            request ??= new BroadcastNotificationRequest();

            var title = string.IsNullOrWhiteSpace(request.title) ? "Broadcast test" : request.title.Trim();
            var body = string.IsNullOrWhiteSpace(request.body)
                ? "Hello from Maison Rider API (broadcast)"
                : request.body.Trim();
            var category = string.IsNullOrWhiteSpace(request.category) ? "system" : request.category.Trim();
            var priority = string.IsNullOrWhiteSpace(request.priority) ? "high" : request.priority.Trim();

            var riders = await _unitOfWork.UserRepository.ListActiveRidersForStoreAsync(request.storeId);
            ...
            return Ok(
                new SendNotificationResultDto
                {
                    ...
                    userIds = userIds,
                    message = string.IsNullOrWhiteSpace(request.storeId)
                        ? $"Broadcast sent to {userIds.Count} active rider(s)"
                        : $"Broadcast sent to {userIds.Count} active rider(s) for store {request.storeId}"
                },
                "Broadcast sent");
        }
        ...
        private static ApiResponse<SendNotificationResultDto> FailResult(string message)
